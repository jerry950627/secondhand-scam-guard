...
1. **離線規則引擎（永遠執行）**：pseudo-query → HyDE → **混合 RAG（詞彙＋語意）** → 分級，完全離線、可解釋、附 165／警政署等真實來源引用。輸入會先經過正規化（全形轉半形、拆字規避還原如「金．流．驗．證」、同義詞如「轉帳→匯款」「加賴→LINE」），中文以 bigram 斷詞讓詞彙重疊在中文上生效。**HyDE 假想文件會被嵌入並真的參與檢索**（可選 `bge-m3`，未拉模型即回退詞彙檢索）。
2. **LLM 語意研判（可選）**：抓規則漏掉的新話術與規避手法；**信心加權融合**——LLM 只在信心足夠時才升級風險、且**永不降級**（降低誤判）。連不到就自動降級為純規則模式。**離線版**用本機 `gemma3:4b`、**線上版**用雲端 Gemini `gemini-2.5-flash`（並支援看截圖 VLM）。
3. **穩健性**：被分析的交易文字以界定符包住、system 指示 LLM 忽略其中操弄指示；且**規則層完全不經 LLM、對提示注入免疫**，是混合式的可靠性賣點。

## 主要功能

- **16 類詐騙知識庫**：假買家、假客服金流驗證、釣魚物流、私下交易、異常低價、**解除分期付款（台灣最常見）**、貨到付款換包裹、假投資、代購代儲，外加假貸款、一頁式廣告、釣魚簡訊、中獎贈品、假冒官方 LINE、假冒檢警等（**其中 6 類取自 data.gov.tw「165 反詐騙詐騙闢謠專區」政府開放資料**），以及正常交易守則參考。
...
- **防詐副駕 — 詐騙攻擊鏈**：把對話對應到 4 階段詐騙漏斗（建立接觸 → 製造急迫 → 誘導離開平台 → 金流收割），畫成時間軸並**預測對方下一步**，讓你看穿整套劇本。
- **防詐副駕 — 安全回覆腳本**：依風險與手法，生成**可一鍵複製、直接貼回去反測對方**的安全話術（堅持平台內交易、拒絕站外付款與金流驗證）。從「偵測」延伸到「主動應對」。
- **QR Code 解析**：截圖裡的收款／釣魚 QR Code 一鍵解析（前端 jsQR），解出的網址直接送進上面的連結黑名單查核。
- **詐騙樣態圖鑑**：可瀏覽 16 類詐騙樣態（話術關鍵詞、防詐建議、官方來源連結、政府開放資料標示），平時也能當學習素材。
- **檢舉與報案管道**：報告附蝦皮／Facebook／Threads 檢舉與 165 報案一鍵連結。
- **明顯免責聲明**：清楚標示僅供風險提醒、不保證準確、不構成法律或金融建議。
...
- `content/anti_fraud_knowledge_base.json`：RAG 知識庫資料（16 類，含 6 類政府開放資料與正常交易參考）。
...
- `tests/test_scam_guard.py`：pytest 測試（70 項）。
